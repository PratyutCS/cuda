...
int main() {
    // Example inputs (hexadecimal numbers as strings)
    ...
    // Convert input using GMP (only for conversion, not for multiplication)
    ...
    // Convert Karatsuba result to GMP number
    ...
    // Compare Karatsuba and GMP results using mpz_cmp
    if (mpz_cmp(karatsuba_product, mpz_result) == 0) {
        cout << "Comparison Result: Karatsuba multiplication matches GMP multiplication!" << endl;
    } else {
        cout << "Comparison Result: Mismatch between Karatsuba and GMP multiplication results!" << endl;
    }
