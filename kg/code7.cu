...
int main() {
    ...
    for (int i = 1; i < row; i++) {
        ...
        for (int j = 0; j < n_line_size; j++) {
            int idx1 = j * 2;
            int idx2 = (j * 2) + 1 < prev_line_size ? (j * 2) + 1 : idx1;
            
            size_t s1 = s[i-1][idx1];
            size_t s2 = s[i-1][idx2];
            
            s[i][j] = s1 + s2;  // Size of result will be sum of operand sizes
            
            // Add memory needed for input arrays and result
            total_mem_needed += (s1 + s2 + (s1+s2)) * sizeof(uint32_t);
        }
        ...
        if (total_mem_needed > max_usable_mem) {
            ...
            // Arrays to store device pointers
    ...
    if (row > 0) {
        ...
        // printf("Result (first 100 chars): %.100s...\n", result_hex);
        // free(result_hex);
    }
